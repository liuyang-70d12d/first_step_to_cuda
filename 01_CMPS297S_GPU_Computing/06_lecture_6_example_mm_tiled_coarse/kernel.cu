...
__global__ void mm_tiled_coarse_kernel(float *A, float *B, float *C, unsigned int M, unsigned int N, unsigned int K)
{
...
void mm_gpu(float *A, float *B, float *C, unsigned int M, unsigned int N, unsigned int K)
{
    ...
    dim3 numBlocks((N + numThreadsPerBlock.x * COARSE_FACTOR - 1) / (numThreadsPerBlock.x * COARSE_FACTOR),
                   (M + numThreadsPerBlock.y - 1) / numThreadsPerBlock.y);
    mm_tiled_coarse_kernel<<<numBlocks, numThreadsPerBlock>>>(A_d, B_d, C_d, M, N, K);
